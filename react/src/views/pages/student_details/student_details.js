...
import Table_fx from '../table_fx/table_fx'
...
class Student_details extends Component {
  ...
  constructor(props) {
    ...
    this.state = {studentcourses: [],showModal: false, courseform: '', courses: [{'course': {'name': ''}}] , data : this.props.location.state.data , originaldata : this.props.location.state.data ,value: '', file: '', showModal: false, redirect:false, newitem: this.props.location.state.newstudent};
...
getcourses = () => {
  ...
  axios.get(urls.api + 'dashboard/' + 'student/'+ this.state.data.id )
  .then( (response) => {
    console.log(response.data.register.map(x => x.course), "ffffff");
    this.setState({studentcourses : response.data.register.map(x => x.course)})
...
modalToggle(value){
  ...
  onChangecourse(e) {
    this.setState({courseform: e.target.value })

    let select = e.target 
    this.setState({course : select.children[select.selectedIndex].id})
...
handleSubmit(e) {
  ...
  delete formData.id
  formData.registration_date = formData.registration_date + 'T00:00:00'
  formData.birth_date = formData.birth_date + 'T00:00:00'
  if (this.state.newitem == true){
    const res =  axios.post(urls.api + 'dashboard/student/' , formData);
    
  } else {
    const res =  axios.put(urls.api + 'dashboard/student/' + this.state.data.id + '/', formData);
...
handleSubmitcourse(e) {
  console.log('dfd', this)
  var formData = {'student': this.state.data.id, 'course': this.state.course }
    
  const res =  axios.post(urls.api + 'dashboard/register/' , formData);
...
componentDidMount() {
 this.getcourses()
  ...
  axios(urls.api + 'dashboard/course/').then( (response) => {
...
handleReset(e) {
  ...
  render(){ 
    let planets = this.state.courses;
    console.log(planets)
          ...
          <CCard>
            ...
            <CCardBody>
              <CFormGroup>
                ...
                <CInput id="name"  value={this.state.data.name} onChange={(value) => this.onChange(value)} />
              </CFormGroup>
              <CFormGroup>
                <CLabel htmlFor="vat">National ID</CLabel>
                <CInput id="national_id"  value={this.state.data.national_id} onChange={(value) => this.onChange(value)} />
              </CFormGroup>
              <CFormGroup>
                <CLabel htmlFor="street">Date of Birth</CLabel>
                <CInput type="date" id="birth_date" name="date-input"  value={this.state.data.birth_date} onChange={(value) => this.onChange(value)}  />
              </CFormGroup>
              
              <CFormGroup>
                <CLabel htmlFor="country">Email</CLabel>
                <CInput id="email"   value={this.state.data.email} onChange={(value) => this.onChange(value)} />
              </CFormGroup>
              <CFormGroup>
                <CLabel htmlFor="country">Registration Date</CLabel>
                <CInput type="date" id="registration_date" name="date-input"  value={this.state.data.registration_date} onChange={(value) => this.onChange(value)}  />
              </CFormGroup>
            </CCardBody>
            <CCardFooter>
              <CButton onClick={this.handleSubmit} type="submit" size="sm" color="primary"><CIcon name="cil-scrubber" /> Submit</CButton> <CButton onClick={this.handleReset} type="reset" size="sm" color="danger"><CIcon name="cil-ban" /> Reset</CButton>
            </CCardFooter>
          </CCard>
        </CCol>
        <CCol xs="12" sm="6">
<CCard>
          ...
          <Table_fx  data = {this.state.studentcourses} redirect={'/courses'} name={'courses'} 
        ...
        <CSelect custom name="relays"  onChange={(value) => this.onChangecourse(value)} >
                    {optionItems}
      
                    </CSelect>
          </CCardBody>
            <CCardFooter>
              <CButton onClick={this.handleSubmitcourse} type="submit" size="sm" color="primary"><CIcon name="cil-scrubber" /> Submit</CButton> 
            </CCardFooter>
        </CCard>
